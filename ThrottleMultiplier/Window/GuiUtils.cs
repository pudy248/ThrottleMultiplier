...
namespace ThrottleMultiplier.Window
{
    ...
    public static class GuiUtils
    {
        ...
        {
            ThrottleMultiplierer.Log("ReadTexture");
            ...
            ThrottleMultiplierer.Log("  Readpixels");
            myTexture2D.ReadPixels(new Rect(0, 0, tmp.width, tmp.height), 0, 0);
            myTexture2D.Apply();
            // Reset the active RenderTexture
            RenderTexture.active = previous;
            // Release the temporary RenderTexture
            ThrottleMultiplierer.Log("  Release");
        ...
        {
            ThrottleMultiplierer.Log("CombineTextures");
            Texture2D baseTexture = ReadTexture(aBaseTexture);
            if (baseTexture == null)
            {
                ThrottleMultiplierer.Log("read texture failed");
                return aBaseTexture;
            }
            ...
            ThrottleMultiplierer.Log("create arrays {0}:{1} {2}", aWidth.ToString(), aHeight.ToString(), aToCopyTexture.width);
            ...
            ThrottleMultiplierer.Log("SetPixels");
            ...
            get
            {
                if (_lockImageOn == null)
                {
                    ...
                    if (!ToolbarControl.LoadImageFromFile(ref _lockImageOn, KSPUtil.ApplicationRootPath + "GameData/GravityTurn/PluginData/Textures/lock_on.png"))
                        ThrottleMultiplierer.Log("could not load lock_on icon!");
                }
                return _lockImageOn;
            }
        }
            ...
            get
            {
                if (_lockImageOff == null)
                {
                    ...
                    if (!ToolbarControl.LoadImageFromFile(ref _lockImageOff, KSPUtil.ApplicationRootPath + "GameData/GravityTurn/PluginData/Textures/lock_off.png"))
                        ThrottleMultiplierer.Log("could not load lock_off icon!");
                }
            ...
            get
            {
                if (_saveIcon == null)
                {
                    ...
                    if (!ToolbarControl.LoadImageFromFile(ref _saveIcon, KSPUtil.ApplicationRootPath + "GameData/GravityTurn/PluginData/Textures/save"))
                        ThrottleMultiplierer.Log("could not load save icon!");
                }
                return _saveIcon;
            }
        }
